...
export default function FavList() {
  ...
  const getAllItem = async () => {
    try {
      const preValue =
        JSON.parse(await AsyncStorage.getItem("@recipe_app_15")) || [];
      setRecipe(preValue);
    } catch (error) {
      console.error(error);
    }
  ...
  const removeFromList = async (item) => {
    ...
    await AsyncStorage.setItem("@recipe_app_15", JSON.stringify(newRecipes));
      ...
      {recipe.length === 0 ? (
        ...
        >
          <Text style={styles.empty}>
            Currently you have nothing in your Favourite List : (
          </Text>
        </View>
      ) : (
